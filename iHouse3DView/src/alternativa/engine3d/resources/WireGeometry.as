...
package alternativa.engine3d.resources {
	...
	public class WireGeometry extends Resource {
         ...
         * @private
         */
		alternativa3d function getDrawUnits(camera:Camera3D, color:Vector.<Number>, thickness:Number, object:Object3D, shader:ShaderProgram, layer:Boolean = false):void {
			for (var i:int = 0; i <= currentSetIndex; i++) {
				...
				if (iBuffer != null && vBuffer != null) {
					...
					if (color[3] < 1) {
						...
						if(layer == false)
							...
							camera.renderer.addDrawUnit(drawUnit, Renderer.NEXT_LAYER);
					}
				}
			}
		}
