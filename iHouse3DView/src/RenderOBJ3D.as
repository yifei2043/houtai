...
{	
	...
	public class RenderOBJ3D extends SpriteVisualElement
	{
		...
		private static var _instance:RenderOBJ3D;
		private var cameraContainer:Object3D;
		
		public var gMain:ModelView;
		...
		public function RenderOBJ3D(v:Single)
		{
			if(!stage)
			{
		...
		{			
			...
			//	var grid:GroundGrid = new GroundGrid(5000,5000);
			//	rootContainer.addChild(grid);
			
			m_CoordCross = CreateReferenceLines(100, 100, 0xBBBBBB);
		...
		public function loadModelComplete(e:Event):void 
		{
			...
			var str:String = m_strPathFile.slice(m_strPathFile.length-3);
			if( str.toLocaleLowerCase() == "a3d" )
			{
				LoadA3D(e);
			}
			
			if( str.toLocaleLowerCase() == "3ds")
			{
				Load3DS(e);
			}	
		}
		...
		public static function getInstance():RenderOBJ3D
		{
			if(_instance==null) _instance = new RenderOBJ3D(new Single());
			return _instance;
		}
	}
}
